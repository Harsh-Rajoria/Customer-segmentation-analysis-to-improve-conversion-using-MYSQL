...
-- Inspect TRANSACTION_LINE table
SELECT * FROM TRANSACTION_LINE;

-- Inspect USER_DATA table
...
SELECT "SANCTIONED_AMOUNT", "ACCOUNT_BALANCE"
FROM TRANSACTION_LINE
...
BEGIN
    BEGIN
        ALTER TABLE TRANSACTION_LINE ADD COLUMN FICO_SEGMENT VARCHAR(20);
    EXCEPTION
        WHEN duplicate_column THEN
            -- Do nothing, column already exists
            RAISE NOTICE 'Column "FICO_SEGMENT" already exists, skipping ADD COLUMN.';
    END;
END;
$$;

-- Update the FICO_SEGMENT column based on FICO_SCORE values
...
-- Account Activity Segmentation Count
...
-- Create a cohort to reveal insights into customer retention and the life cycle of accounts. 
...
-- Identify Common Account Combinations: Look at customers who have multiple account types and identify common combinations.
